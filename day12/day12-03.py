"""
클래스 메소드 , 정적 메소드

"""

'''
1. 클래스 변수란?
[인스턴스 변수]
클래스를 구현할 때 인스턴스마다 서로 다른 값을 가지는 경우
-> 인스턴스 변수를 사용했다.
-> self 키워드를 붙여서 사용

[클래스 변수]
인스턴스들이 서로 같은 값을 가지는 변수 

예시) 한국 사람을 클래스로 구현한다
[인스턴스 변수]
-이름, 나이, 주소, 등

[클래스 변수]
-한국 사람

[클래스 변수 사용하는 이유]
-메모리 공간의 낭비를 막기 위해서!
# 좋은 개발자 
-> 메모리 낭비 덜하기
-> 속도

'''
#
# class Korea:
#     # 클래스 변수 country
#     country ='한국'
#     def __init__(self,name,age,address):
#         self.name = name
#         self.age = age
#         self.address= address
# man = Korea('홍길동',35,'서울')
# print(man.name)
# print(Korea.name)
# #Korea.name 은 에러가 발생합니다 Korea 라는 객체에는 name 이라는 것이 없기 때문에
#
# print(man.country)
# print(Korea.country)

'''
[구분]

인스턴스 변수
 - 목적 : 인스턴스 마다 다른 값을 저장
 -접근 방식 : 인스턴스 접근 (o)
            클래스 직접 접근(X)
클래스 변수 
    - 목적 : 인스턴스가 공유하는 값을 저장
    - 접근 방식 : 인스턴스 접근 ( o )
                클래스 직접 접근 ( o )
    * 클래스 변수는 클래스를 통해서 접근하는 것이 권장 사항!
 
'''


'''
2.클래스 메소드 
클래스 메소드 = 클래스 변수를 사용하는 메소드


[클래스 메소드의 주요 특징]
- 인스턴스 또는 클래스로 호출합니다!
- 생성된 인스턴스가 없어도 호출 가능합니다!
- @classmethod 데코레이터를 표시하고 작성합니다
- 매개변수 self ( x ) - > cls 를 사용합니다.

[호출 방식의 차이]
-인스턴스를 통해서 호출 X -> 클래스.메소드() 와 같은 형식으로 호출!
-self 사용 X -> cls를 통해서 클래스 변수에 접근
    ->> 인스턴스 메소드와 가장 큰 차이점.
'''

class Korea:
    # 클래스 변수 county
    country = '한국'

    # 클래스 메소드라고 알려주는 데코레이터
    @classmethod
    def trip(cls,country):
        #cls == class == 현재 여기에선 Korea class
        #cls.country == korea.country
        if cls.country == country:
            print('국내여행입니다')
        else:
            print('해외여행입니다.')
Korea.trip('한국')
Korea.trip('미국')


import datetime

#date time 클래스 메소드
print(datetime.datetime.now())


'''
3. 정적 메소드
self 사용 하지 않는다! -> 인스턴스변수를 사용할 수 없다
cls 도 사용하지 않는다.

정적 메소드의 특징
- 인스턴스 또는 클래스 호출
- 생성된 인스턴스가 없어도 호출
- @staticmethod 데코레이터를 표시하고 작성
- 반드시 작성해야 할 매개변수 X

즉!
self,cls X -> 인스턴스에 영향을 주지 않고 인스턴스로부터 영향을 받지도 않는다!

'''

class Korea:

    country = '한국'
    # 정적인 메소드로 사용한다!
    @staticmethod
    def slogan():
        # 인스턴스 변수, 클래스 변수 모두 사용하지 않는다!
        print('Imagine your Korea')

# 인스턴스 생성 없이 호출할 수 있는게 장점
Korea.slogan()

'''

[정적인 메소드 사용처]
-> Util 도구 모음 할 때 사용 많이한다!
ex) 자바에 isEmpty 메소드가 있는데
    -> () 안에 들어온 리스트가 null or length == 0 > true
    
a = ''
     
'''

'''
가방을 만들 떄 마다 현재 만들어진 갑아이 몇 개인지 계산할 수 있는 Bag 클래스 이다.

'''

class Bag:
    count = 0

    def __init__(self):
        Bag.count +=1

        # 클래스 메소드
    @classmethod
    def sell(cls):
        cls.count -=1

    @classmethod
    def remain_bag(cls):
        return cls.count

print(f'현재 가방{Bag.remain_bag}개')
# bag1 = Bag()
Bag()
Bag()
Bag()
print(f'현재 가방{Bag.remain_bag()}개')
Bag.sell()
Bag.sell()
print(f'현재 가방{Bag.remain_bag()}개')
