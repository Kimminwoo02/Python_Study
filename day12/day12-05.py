...
1. 다음 지시사항을 읽고 이름과 전체 인구수를 
저장할 수 있는Person 클래스를 생성하세요

[지시 사항]
-다음과 같은 방법으로 man 과 woman 인스턴스를 생성하세요
man = Person('James')
woman = Person('emily')

-man 과 woman 인스턴스가 생성되면 다음과 같은 메세지를 출력할 수 있도록 처리하세요
...
print(f'전체인구수:{Person.get_population()}명') 전체 인구수 2명

-다음과 같은 방법으로 man 인스턴스를 삭제하세요

-man 인스턴스가 삭제되면 다음과 같은 메세지를 출력할 수 잇도록 처리하세요
james is dead
...
[지시사항]
-shop 클래스는 다음과 같은 클래스 변수를 가지고 있습니다.
...
[지시사항]
다음과 같은 슈퍼클래스 Car를 가지고 있는 서브 클래스 Hybrid를 구현하세요.
class car:
    max_oil = 50# 최대 주유량
...
-서브 클래스 Hybrid 는 다음과 같은 특징을 가지고 있습니다.
    ㄴ 최대 배터리 충전량은 30 입니다. 
    ㄴ 배터리 충전하는 charge() 메소드가 있습니다.
        - 최대 충전량을 초과하도록 충전할 수 없고, 0보다 작은 값으로 충전을 
        할 수 없습니다.
    ㄴ 현재 주유량과 충전량을 모두 할 수 있는 hybrid_info() 메소드가 있습니다.
[동작]
car = Hybrid(0,0)
car.add_oil(100)
car.charge(50)
