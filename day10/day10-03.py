...
디렉토리 우클릭 -> 오픈인 -> 익스프로러 클릭시 경로가 나온다! 
...
#
# #전체 경로로 파일열기! [절대 경로]
# open('C:/Users/made_/PycharmProjects/pythonProject/day10/test.txt') # \ 이 아니고 /로 바꾼다!
#
...
'''
2-2 모드
모드란 파일을 여는 목적이라고 볼 수 있다!
파일 입력을 위해서인지 출력을 위해서인지 모드를 통해 결정합니다!
...
- 출력 
    ...
    ㄴ a: 추가
    ㄴ x: 배타적 추가 (잘 안씀)
* 입출력 모드를 생략하면 기본저그올 r 모드로 파일이 열린다!
* 출력을 목적으로 한다면 해당 모드를 명시해야한다!

---

파일을 여는 목적을 결정했다면 어떤 파일인지 파일의 종류도 구분해야한다!
일반적으로 열러고 하는 파일이 텍스트 파일인가 아닌가? 요걸로 결정해야한다!
[파일 종류]
- t : 텍스트 파일
- b : 바이너리 파일

* 열고자 하는 파일의 종류를 생각하면 기본적으로 t모드(텍스트 모드)로 파일이 열림!
* 바이너리 파일인 경우 반드시 b모드 명시!

[t]
텍스트 모드는 문자를 대상을 입출력을 처리하기 때문에,
영문이나 한글등으로 작성된 텍스트 파일에서 사용함
...
입출력을 처리하기 때문에 텍스트를 사용하지 않는 모든  파일(이미지, 사운드, 동영상)  
사용하는 모드
...
'''
2.파일 닫기
파일을 더 이상 사용하지 않거나 프로그램을 종료하고자 할 때 열어놓은 파일을 꼭! 닫아주는게 좋다!
...
예외처리가 아니더라도 python에서는 close() 메소드를 자동으로 호출 할 수있는 
with가 존재합니다!
